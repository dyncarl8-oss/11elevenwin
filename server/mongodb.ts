...
export async function connectMongoDB(): Promise<Db> {
  ...
  try {
    ...
    // Clean up crosshair skins (no longer sold)
    await cleanupCrosshairSkins();
...
async function seedSkins(): Promise<void> {
  ...
  // Clean up any crosshair skins that may exist from before
  await cleanupCrosshairSkins();
}

// Remove crosshair skins from the database (no longer sold)
export async function cleanupCrosshairSkins(): Promise<void> {
  if (!db) return;
  
  const skinsCollection = db.collection("skins");
  const userSkinsCollection = db.collection("user_skins");
  
  // Delete crosshair skins from store
  const deletedSkins = await skinsCollection.deleteMany({ category: "crosshair" });
  if (deletedSkins.deletedCount > 0) {
    console.log(`Removed ${deletedSkins.deletedCount} crosshair skins from store`);
  }
  
  // Delete crosshair skins from user inventories
  const deletedUserSkins = await userSkinsCollection.deleteMany({ category: "crosshair" });
  if (deletedUserSkins.deletedCount > 0) {
    console.log(`Removed ${deletedUserSkins.deletedCount} crosshair skins from user inventories`);
  }
}
